//! Connection handler implementation for substream protocols.
//!
//! This mod contains the connection handler that manages individual peer connections,
//! protocol negotiations, and substream establishment. It processes both inbound and
//! outbound stream requests and handles protocol upgrades.
use std::{
...
use libp2p::{
    ...
    InboundUpgrade, OutboundUpgrade, PeerId, Stream, StreamProtocol,
};
use smallvec::SmallVec;

use crate::{
    error::Error, event::Event, substream_types::OpenStreamRequest, FromBehaviorEvent,
    ProtocolEvent, ProtocolNotification, StreamId, MAX_IDLE_QUEUE_SIZE,
};

/// Connection handler for managing individual peer connections and their substreams.
/// Responsible for protocol negotiation and stream establishment.
pub struct Handler {
    peer_id: PeerId,
    protocols: Protocols<StreamProtocol>,
    requested_streams: VecDeque<OpenStreamRequest>,
    pending_behavior_events: VecDeque<FromBehaviorEvent>,
...
impl Handler {
    pub fn new(peer_id: PeerId, protocols: SmallVec<[StreamProtocol; 32]>) -> Self {
        Self {
            ...
            pending_behavior_events: VecDeque::new(),
...
impl Handler {
    ...
    fn on_dial_upgrade_error(
        &mut self,
        error: DialUpgradeError<StreamId, ChosenProtocol<StreamProtocol>>,
    ) {
        let stream = self
            .requested_streams
            .pop_front()
...
impl ConnectionHandler for Handler {
    type FromBehaviour = FromBehaviorEvent;
    ...
    > {
        ...
        if let Some(event) = self.pending_behavior_events.pop_front() {
            match event {
                FromBehaviorEvent::OpenRpcSessionRequest(stream) => {
                ...
                FromBehaviorEvent::AddSupportedProtocol(added) => {
        ...
        debug_assert!(self.pending_behavior_events.is_empty());

        if self.pending_behavior_events.capacity() > MAX_IDLE_QUEUE_SIZE {
            self.pending_behavior_events.shrink_to_fit();
        }
    ...
    fn on_behaviour_event(&mut self, stream: Self::FromBehaviour) {
        self.pending_behavior_events.push_back(stream);
    }
